...
# Преобразуем координаты для отрисовки полилинии (для отображения на изображении)
...
while cv2.waitKey(1) != 27:  # Продолжаем, пока не нажата клавиша 'Esc'
    ...
    binary[(r_channel > 200)] = 1  # Устанавливаем пиксели как 1, если они достаточно красные
    ...
    binary2[(s_channel > 160)] = 1  # Устанавливаем пиксели как 1, если S-канал выше порога
